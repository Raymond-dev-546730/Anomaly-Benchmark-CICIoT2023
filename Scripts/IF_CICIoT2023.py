...
def train_isolation_forest(X_train, X_val, seed, memory_limit_bytes):

    print(f"Training Isolation Forest (seed {seed})...")
    ...
    model = IsolationForest(
        ...
        max_features=1.0,
        bootstrap=False,
        n_jobs=HARDWARE_CONFIG['num_threads'],
        random_state=seed,
        verbose=0
    )
...
def run_seed_experiment(benign_df, seed, memory_limit_bytes):
    ...
    model, threshold, training_metrics = train_isolation_forest(X_train_norm, X_val_norm, seed, memory_limit_bytes)

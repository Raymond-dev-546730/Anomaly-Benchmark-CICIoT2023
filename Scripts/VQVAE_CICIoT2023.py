...
class VQVAE_Autoencoder(nn.Module):

    def __init__(self, input_dim, hidden_dim=32, num_embeddings=512, commitment_cost=0.25):
        super(VQVAE_Autoencoder, self).__init__()
...
def measure_model_loading_time(model, device):
    ...
    model_copy = VQVAE_Autoencoder(model.encoder[0].in_features, hidden_dim=32)
    ...
    for _ in range(num_runs):
        ...
        test_model = VQVAE_Autoencoder(model.encoder[0].in_features, hidden_dim=32)
...
def train_autoencoder(X_train, X_val, seed, device, memory_limit_bytes):
    ...
    print(f"Training Vector-Quantized Variational Autoencoder (seed {seed})...")
    ...
    model = VQVAE_Autoencoder(X_train.shape[1], hidden_dim=32)
    model.to(device)
    ...
    for epoch in range(50):
        ...
        for i in range(0, len(X_train), batch_size):
            ...
            reconstructed, vq_loss, indices = model(batch)

...
class DenoisingAutoencoder(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=32, corruption_level=0.2):
        ...
        # Decoder (progressively restore dimensions)
    ...
    def add_masking_corruption(self, x, training=True):

        if training and self.corruption_level > 0:

            mask = torch.bernoulli(torch.full_like(x, 1 - self.corruption_level))
            return x * mask
        return x
    
    def forward(self, x, add_corruption=True):
        # Add masking corruption during training for denoising effect
        if self.training and add_corruption:
            corrupted_x = self.add_masking_corruption(x)
        else:
            corrupted_x = x
...
def measure_model_loading_time(model, device):
    ...
    model_copy = DenoisingAutoencoder(
    ...
    for _ in range(num_runs):
        ...
        test_model = DenoisingAutoencoder(
            model.encoder[0].in_features, 
            hidden_dim=32, 
            corruption_level=0.2
        )
...
def train_autoencoder(X_train, X_val, seed, device, memory_limit_bytes):
    print(f"Training Denoising Autoencoder (seed {seed})...")
    ...
    # Initialize model with corruption level for masking
    model = DenoisingAutoencoder(X_train.shape[1], hidden_dim=32, corruption_level=0.2)
    model.to(device)
    ...
    print(f"Corruption level (masking): {model.corruption_level:.1%}")
    ...
    for epoch in range(50):
        ...
        for i in range(0, len(X_train), batch_size):
            ...
            # Forward pass with masking corruption
            reconstructed, encoded = model(batch, add_corruption=True)
            
            # Loss is reconstruction error between clean input and output
        ...
        # Validation phase (no corruption)
        model.eval()
        val_loss = 0
        val_batch_count = 0
        ...
        with torch.no_grad():
            for i in range(0, len(X_val), batch_size):
                ...
                # Forward pass without corruption for validation
                reconstructed, encoded = model(batch, add_corruption=False)
                loss = criterion(reconstructed, batch)
    ...
    model.eval()
    with torch.no_grad():
        val_reconstructed, _ = model(X_val.to(device), add_corruption=False)
        val_errors = torch.mean((X_val.to(device) - val_reconstructed) ** 2, dim=1)
...
def evaluate_model(model, scaler, device, memory_limit_bytes):
    ...
    all_errors = []
    all_labels = []
    model.eval()  # No corruption during evaluation
    ...
    for i, partition in enumerate(partitions):
        ...
        # Controlled inference with timing (no masking corruption)
        inference_start = time.perf_counter()
        with torch.no_grad():
            reconstructed, encoded = model(X_tensor.to(device), add_corruption=False)
...
def calculate_metrics(errors, labels, threshold):
    ...
    # threshold provided from validation set  

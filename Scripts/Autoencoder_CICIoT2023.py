...
def measure_model_loading_time(model, device):
    ...
    model_copy = Autoencoder(model.encoder[0].in_features, hidden_dim=32)
    ...
    for _ in range(num_runs):
        ...
        test_model = Autoencoder(model.encoder[0].in_features, hidden_dim=32)
...
def train_autoencoder(X_train, X_val, seed, device, memory_limit_bytes):
    ...
    model = Autoencoder(X_train.shape[1], hidden_dim=32)
    model.to(device)
    ...
    model.eval()
    with torch.no_grad():
        val_errors = torch.mean((X_val.to(device) - model(X_val.to(device))) ** 2, dim=1)
...
def evaluate_model(model, scaler, device, memory_limit_bytes):
    ...
    for i, partition in enumerate(partitions):
        ...
        with torch.no_grad():
            errors = torch.mean((X_tensor.to(device) - model(X_tensor.to(device))) ** 2, dim=1)

...
def train_gaussian_mixture(X_train, X_val, seed, memory_limit_bytes):

    print(f"Training Gaussian Mixture Model (seed {seed})...")
    ...
    val_log_likelihood = model.score_samples(X_val)
    val_scores = -val_log_likelihood
...
def evaluate_model(model, scaler, memory_limit_bytes):
    ...
    for i, partition in enumerate(partitions):
        ...
        # Anomaly score
        log_likelihood = model.score_samples(X_norm)
...
def run_seed_experiment(benign_df, seed, memory_limit_bytes):
    ...
    model, threshold, training_metrics = train_gaussian_mixture(X_train_norm, X_val_norm, seed, memory_limit_bytes)

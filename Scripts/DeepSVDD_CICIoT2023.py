...
class DeepSVDDNetwork(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=32):
        ...
        # Network without bias terms (required to prevent hypersphere collapse)
...
def initialize_center(model, data_loader, device, eps=0.1):

    model.eval()
    n_samples = 0
    c = torch.zeros(model.hidden_dim, device=device)
    ...
    # If center is too close to zero, set to eps to avoid hypersphere collapse
    c[(abs(c) < eps) & (c < 0)] = -eps
    c[(abs(c) < eps) & (c >= 0)] = eps
    
    return c

def train_deep_svdd(X_train, X_val, seed, device, memory_limit_bytes):
    print(f"Training Deep SVDD (seed {seed})...")
    ...
    # Create data loaders for center initialization
    ...
    center = initialize_center(model, train_loader, device)
    print(f"Center initialized with norm: {torch.norm(center):.4f}")
    ...
    with torch.no_grad():
        ...
        threshold = np.percentile(val_distances.cpu().numpy(), 99)
...
def evaluate_model(model, center, scaler, device, memory_limit_bytes):
    ...
    for i, partition in enumerate(partitions):
        ...
        with torch.no_grad():
            ...
            all_errors.extend(distances.cpu().numpy())
...
def run_seed_experiment(benign_df, seed, device, memory_limit_bytes):
    ...
    model, center, threshold, training_metrics = train_deep_svdd(X_train_norm, X_val_norm, seed, device, memory_limit_bytes)
    errors, labels, evaluation_metrics = evaluate_model(model, center, scaler, device, memory_limit_bytes)
